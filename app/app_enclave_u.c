...
typedef struct ms_enclaveLoadVal_t {
	float ms_retval;
} ms_enclaveLoadVal_t;

typedef struct ms_encrypt_result_t {
	int ms_retval;
} ms_encrypt_result_t;

typedef struct ms_payload_to_buf_t {
	uint8_t* ms_buf;
	size_t ms_len;
} ms_payload_to_buf_t;

typedef struct ms_mac_tag_to_buf_t {
	sgx_aes_gcm_128bit_tag_t* ms_buf;
	size_t ms_len;
} ms_mac_tag_to_buf_t;
...
static sgx_status_t SGX_CDECL app_enclave_create_session_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL app_enclave_exchange_report_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL app_enclave_close_session_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL app_enclave_invoke_service_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL app_enclave_sgx_oc_cpuidex(void* pms)
{
...
static sgx_status_t SGX_CDECL app_enclave_sgx_thread_wait_untrusted_event_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL app_enclave_sgx_thread_set_untrusted_event_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL app_enclave_sgx_thread_setwait_untrusted_events_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL app_enclave_sgx_thread_set_multiple_untrusted_events_ocall(void* pms)
{
...
static const struct {
	size_t nr_ocall;
	void * table[9];
} ocall_table_app_enclave = {
	9,
	{
		(void*)app_enclave_create_session_ocall,
		(void*)app_enclave_exchange_report_ocall,
		(void*)app_enclave_close_session_ocall,
		(void*)app_enclave_invoke_service_ocall,
		(void*)app_enclave_sgx_oc_cpuidex,
		(void*)app_enclave_sgx_thread_wait_untrusted_event_ocall,
		(void*)app_enclave_sgx_thread_set_untrusted_event_ocall,
		(void*)app_enclave_sgx_thread_setwait_untrusted_events_ocall,
		(void*)app_enclave_sgx_thread_set_multiple_untrusted_events_ocall,
	}
};
sgx_status_t enclave_init_ra(sgx_enclave_id_t eid, sgx_status_t* retval, int b_pse, sgx_ra_context_t* p_context)
{
	...
	status = sgx_ecall(eid, 0, &ocall_table_app_enclave, &ms);
...
sgx_status_t enclave_ra_close(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context)
{
	...
	status = sgx_ecall(eid, 1, &ocall_table_app_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t verify_att_result_mac(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, uint8_t* message, size_t message_size, uint8_t* mac, size_t mac_size)
{
	...
	status = sgx_ecall(eid, 2, &ocall_table_app_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t put_secret_data(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, uint8_t* p_secret, uint32_t secret_size, uint8_t* gcm_mac)
{
	...
	status = sgx_ecall(eid, 3, &ocall_table_app_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t enclaveChangeBuffer(sgx_enclave_id_t eid, char* buf, size_t len)
{
	...
	ms.ms_buf = buf;
	ms.ms_len = len;
	status = sgx_ecall(eid, 4, &ocall_table_app_enclave, &ms);
	return status;
}
...
sgx_status_t enclaveStringSave(sgx_enclave_id_t eid, char* input, size_t len)
{
	...
	ms.ms_input = input;
	ms.ms_len = len;
	status = sgx_ecall(eid, 5, &ocall_table_app_enclave, &ms);
	return status;
}
...
sgx_status_t enclaveStringLoad(sgx_enclave_id_t eid, char* output, size_t len)
{
	...
	ms.ms_output = output;
	ms.ms_len = len;
	status = sgx_ecall(eid, 6, &ocall_table_app_enclave, &ms);
	return status;
}

sgx_status_t enclaveLoadVal(sgx_enclave_id_t eid, float* retval)
{
	sgx_status_t status;
	ms_enclaveLoadVal_t ms;
	status = sgx_ecall(eid, 7, &ocall_table_app_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t encrypt_result(sgx_enclave_id_t eid, int* retval)
{
	sgx_status_t status;
	ms_encrypt_result_t ms;
	status = sgx_ecall(eid, 8, &ocall_table_app_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t payload_to_buf(sgx_enclave_id_t eid, uint8_t* buf, size_t len)
{
	sgx_status_t status;
	ms_payload_to_buf_t ms;
	ms.ms_buf = buf;
	ms.ms_len = len;
	status = sgx_ecall(eid, 9, &ocall_table_app_enclave, &ms);
	return status;
}

sgx_status_t mac_tag_to_buf(sgx_enclave_id_t eid, sgx_aes_gcm_128bit_tag_t* buf, size_t len)
{
	sgx_status_t status;
	ms_mac_tag_to_buf_t ms;
	ms.ms_buf = buf;
	ms.ms_len = len;
	status = sgx_ecall(eid, 10, &ocall_table_app_enclave, &ms);
	return status;
}
...
sgx_status_t sgx_ra_get_ga(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, sgx_ec256_public_t* g_a)
{
	...
	ms.ms_context = context;
	ms.ms_g_a = g_a;
	status = sgx_ecall(eid, 11, &ocall_table_app_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t sgx_ra_proc_msg2_trusted(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, const sgx_ra_msg2_t* p_msg2, const sgx_target_info_t* p_qe_target, sgx_report_t* p_report, sgx_quote_nonce_t* p_nonce)
{
	...
	status = sgx_ecall(eid, 12, &ocall_table_app_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
sgx_status_t sgx_ra_get_msg3_trusted(sgx_enclave_id_t eid, sgx_status_t* retval, sgx_ra_context_t context, uint32_t quote_size, sgx_report_t* qe_report, sgx_ra_msg3_t* p_msg3, uint32_t msg3_size)
{
	...
	status = sgx_ecall(eid, 13, &ocall_table_app_enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
