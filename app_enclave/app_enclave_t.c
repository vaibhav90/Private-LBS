...
static sgx_status_t SGX_CDECL sgx_enclaveLoadVal(void* pms)
{
	ms_enclaveLoadVal_t* ms = SGX_CAST(ms_enclaveLoadVal_t*, pms);
	sgx_status_t status = SGX_SUCCESS;

	CHECK_REF_POINTER(pms, sizeof(ms_enclaveLoadVal_t));

	ms->ms_retval = enclaveLoadVal();


	return status;
}

static sgx_status_t SGX_CDECL sgx_encrypt_result(void* pms)
{
	ms_encrypt_result_t* ms = SGX_CAST(ms_encrypt_result_t*, pms);
	sgx_status_t status = SGX_SUCCESS;

	CHECK_REF_POINTER(pms, sizeof(ms_encrypt_result_t));

	ms->ms_retval = encrypt_result();


	return status;
}

static sgx_status_t SGX_CDECL sgx_payload_to_buf(void* pms)
{
	ms_payload_to_buf_t* ms = SGX_CAST(ms_payload_to_buf_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	CHECK_REF_POINTER(pms, sizeof(ms_payload_to_buf_t));
	CHECK_UNIQUE_POINTER(_tmp_buf, _len_buf);
	...
	payload_to_buf(_in_buf, _tmp_len);
err:
	if (_in_buf) {
		memcpy(_tmp_buf, _in_buf, _len_buf);
		free(_in_buf);
...
static sgx_status_t SGX_CDECL sgx_mac_tag_to_buf(void* pms)
{
	ms_mac_tag_to_buf_t* ms = SGX_CAST(ms_mac_tag_to_buf_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	sgx_aes_gcm_128bit_tag_t* _tmp_buf = ms->ms_buf;
	size_t _tmp_len = ms->ms_len;
	size_t _len_buf = _tmp_len;
	sgx_aes_gcm_128bit_tag_t* _in_buf = NULL;

	CHECK_REF_POINTER(pms, sizeof(ms_mac_tag_to_buf_t));
	CHECK_UNIQUE_POINTER(_tmp_buf, _len_buf);

	if (_tmp_buf != NULL) {
		if ((_in_buf = (sgx_aes_gcm_128bit_tag_t*)malloc(_len_buf)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
	...
	mac_tag_to_buf(_in_buf, _tmp_len);
err:
	if (_in_buf) {
		memcpy(_tmp_buf, _in_buf, _len_buf);
		free(_in_buf);
...
SGX_EXTERNC const struct {
	...
	14,
	{
		...
		{(void*)(uintptr_t)sgx_enclaveLoadVal, 0},
		{(void*)(uintptr_t)sgx_encrypt_result, 0},
		{(void*)(uintptr_t)sgx_payload_to_buf, 0},
		{(void*)(uintptr_t)sgx_mac_tag_to_buf, 0},
